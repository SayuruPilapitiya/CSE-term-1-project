...
import { createBooks, updateBook } from '../app/actions';
...
interface AdvertiseModalProps {
    ...
    firstName: string;
    lastName: string;
    bookToEdit?: any; // Add bookToEdit prop
}
...
export default function AdvertiseModal({ isOpen, onClose, firstName, lastName, bookToEdit }: AdvertiseModalProps) {
    ...
    useEffect(() => {
        if (bookToEdit) {
            setCurrentBook({
                ...
                condition: bookToEdit.condition || 'Used',
                subject: bookToEdit.subject || '',
                description: bookToEdit.description || '',
                extra_details: bookToEdit.extra_details || '',
                medium: bookToEdit.medium || 'Sinhala'
            });
            setBookList([]); // Clear list when editing single item
        } else {
    ...
    const handlePublish = async () => {
        if (!bookToEdit && bookList.length === 0 && (!currentBook.title || !currentBook.price)) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
            <div className={`bg-white rounded-lg w-full ${isEditMode ? 'max-w-xl' : 'max-w-4xl'} m-4 p-6 max-h-[90vh] overflow-y-auto`}>
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-900">
                        {isEditMode ? 'Edit Advertisement' : 'Advertise Your Books'}
                    </h2>
                ...
                <div className={`grid ${isEditMode ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'} gap-8`}>
                    {/* Form Section */}
                    <div className="space-y-4">
                        {!isEditMode && <h3 className="text-lg font-semibold text-gray-700">Add New Book</h3>}
                    ...
                    {/* List Section - Only show if NOT editing */}
                    {!isEditMode && (
                                    ...
                                    bookList.map((book, index) => (
                                        <div key={index} className="bg-white p-4 rounded shadow-sm border border-gray-200 relative">
                                            ...
                                            <div className="flex gap-2 text-xs mt-1">
                                                <span className="bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded">{book.medium}</span>
                                                <span className="bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded">{book.condition}</span>
                                            </div>
